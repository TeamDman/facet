...
fn codegen_java(workspace_root: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    expected.push(java_testbed_echo_constants()?);
...
fn check_codegen_java(workspace_root: &std::path::Path) -> Result<(), Box<dyn std::error::Error>> {
    ...
    generated.push(java_testbed_echo_constants()?);
...
fn java_testbed_echo_constants()
-> Result<vox_codegen::targets::java::JavaFile, Box<dyn std::error::Error>> {
    ...
    let source = format!(
        "// @generated by vox-codegen; DO NOT EDIT.\n\
         ...
         \x20 public static final String SERVICE = \"{}\";\n\
         \x20 public static final long ECHO_METHOD_ID = 0x{:016x}L;\n\
         \x20 private TestbedWireConstants() {{}}\n\
         }}\n",
        service.service_name,
        vox_codegen::method_id(echo),
    );
    Ok(vox_codegen::targets::java::JavaFile {
        relative_path: "TestbedWireConstants.java".to_string(),
        source,
    })
}
