...
final class SchemaWire {
    ...
    private static final class In{
        ...
        int count()throws PhonException{long n=u32();if(n>l.collectionEntries()||n>remaining())throw error("unbounded count "+n);return(int)n;}int length()throws PhonException{long n=u32();if(n>l.byteRunLength()||n>remaining())throw error("unbounded byte run "+n);return(int)n;}String string()throws PhonException{int n=length();need(n);try{String s=StandardCharsets.UTF_8.newDecoder().onMalformedInput(CodingErrorAction.REPORT).decode(ByteBuffer.wrap(b,p,n)).toString();p+=n;return s;}catch(CharacterCodingException e){throw error("invalid UTF-8");}}
